...
const
  DBFileName: String = 'SmartDevicesDB.mdb';
  OneTableName: string = 'tblManufacturers';
  ManyTableName: string = 'tblDevices';
  BackupOneTblName: string = 'CopytblManufacturers';
  BackupManyTblName: string = 'CopytblDevices';
...
begin
  ...
  dbConnection.ConnectionString :=
    ...
    'Jet OLEDB:Database Password=HkNg2!';
  ...
  tblOne.Open;
  tblOne.Sort := 'ManufacturerID ASC';
  tblOne.First;
  ...
  tblMany.Open;
  tblMany.FieldByName('Price').Alignment := taLeftJustify;
  tFloatField(tblMany.FieldByName('Price')).currency := True;
  tblMany.Sort := 'DeviceID ASC';
  tblMany.First;
...
begin
  ...
  begin
    ...
    iBefore := tblMany.RecordCount;

    qryA.Close;
    qryA.SQL.Text := Statement;
    qryA.ExecSQL;
    DisplayAllContent(qryA, 'tblManufacturers');

    tblMany.Close;
    tblMany.Open;

    iAfter := tblMany.RecordCount;

    showMessage('Record(s) deleted: ' + IntToStr(iBefore - iAfter));
...
begin
  ...
  GridOne.Columns[1].Width := 225;
  GridOne.Columns[2].Width := 228;
  GridOne.Columns[3].Width := 175;
  GridOne.Width := 800;
  GridMany.Columns[0].Width := 100;
  GridMany.Columns[1].Width := 120;
  GridMany.Columns[2].Width := 100;
  GridMany.Columns[3].Width := 110;
  GridMany.Columns[4].Width := 100;
  tFloatField(tblMany.FieldByName('Price')).currency := True;
  GridMany.Columns[5].Width := 95;
  GridMany.Columns[5].Alignment := taLeftJustify;
